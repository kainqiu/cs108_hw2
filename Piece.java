...
public class Piece {
	...
	*/
	public Piece(TPoint[] points) {
		...
		// add 1 to width and height, considering index - 1
		this.width++;
		this.height++;
		this.skirt = new int[this.width];
		for(int i = 0; i < this.width; i++) {
			this.skirt[i] = MAX_HEIGHT;	
		}
		for(TPoint tp : points) {
			if(tp.y < this.skirt[tp.x]) {
				this.skirt[tp.x] = tp.y;
			}
		}
		// to do: initialize next
		//this.next = null;
	}
	...
	*/
	...
	public Piece computeNextRotation() {
		// for each point (x, y) -> (this.height - 1 - y, x)
		List<TPoint> rotatedPoints = new ArrayList<TPoint>();
		for(TPoint tp : this.body) {
			rotatedPoints.add(new TPoint(this.height - 1 - tp.y, tp.x));
		}
		TPoint[] rotatedArray = rotatedPoints.toArray(new TPoint[0]);
		return new Piece(rotatedArray);
	...
	*/	
	public Piece fastRotation() {
		if(this.next == null) {
			//return this.computeNextRotation();
			return makeFastRotations(this).next;
		}
		return next;
	}
	...
	*/
	public boolean equals(Object obj) {
		...
		if(bodyLen != other.body.length) return false;
		boolean[] matchCount = new boolean[bodyLen];
		...
		for(int i = 0; i < bodyLen; i++) {
			if(matchCount[i] == false) return false;
		}
		return true;
	}
	...
	*/
	private static Piece makeFastRotations(Piece root) {
		if(root.next == null) {
			Piece rootRotation = root;
			while(true) {
				Piece nextRotation = new Piece(rootRotation.computeNextRotation().body);
				if(nextRotation.equals(root)) {
					rootRotation.next = root;
					break;
				} else {
					rootRotation.next = nextRotation;
				}
				rootRotation = nextRotation;
			}
		}
